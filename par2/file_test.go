...
func (d testDecoderDelegate) OnDetectDataFileHashMismatch(fileID [16]byte, path string, err error) {
	d.t.Helper()
	d.t.Logf("OnDetectDataFileHashMismatch(%x, %s, %v)", fileID, path, err)
}
