...
func TestStandardToAltMapAVX2Unsafe(t *testing.T) {
	skipNonSSSE3(t)
	...
	filler := fill32(0xfe)
	outLow := [2][32]byte{filler, filler}
	outHigh := [2][32]byte{filler, filler}

	expectedOutLow := [2][32]byte{{
		0x20, 0x30, 0x40, 0x50,
		0x60, 0x70, 0x80, 0x90,
		0xa0, 0xb0, 0xc0, 0xd0,
		0xe0, 0xf0, 0x00, 0x10,
	...
	standardToAltMapAVX2Unsafe(&in0, &in1, &outLow[0], &outHigh[0])

	assert.Equal(t, expectedOutLow, outLow)
	assert.Equal(t, expectedOutHigh, outHigh)
}
...
func TestMulSSSE3Unsafe(t *testing.T) {
	skipNonSSSE3(t)
	...
	filler := fill16(0xfa)
	expectedOut0 := [2][16]byte{filler, filler}
	expectedOut1 := [2][16]byte{filler, filler}
	mulByteSliceLEGeneric(c, in0[:], expectedOut0[0][:])
	mulByteSliceLEGeneric(c, in1[:], expectedOut1[0][:])
...
func TestMulAndAddSSSE3Unsafe(t *testing.T) {
	skipNonSSSE3(t)
	...
	expectedOut0 := [2][16]byte{filler, filler}
	expectedOut1 := [2][16]byte{filler, filler}
	mulAndAddByteSliceLEGeneric(c, in0[:], expectedOut0[0][:])
	mulAndAddByteSliceLEGeneric(c, in1[:], expectedOut1[0][:])
...
func TestMulAndAddSliceSSSE3Unsafe(t *testing.T) {
	skipNonSSSE3(t)
